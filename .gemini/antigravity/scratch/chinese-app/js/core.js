  ...
  async sync() {
    ...
    try {
      const fullState = {
        ...
        cards: this.cards,
        dictationPlaylist: this.dictationPlaylist,
      ...
      if (error) throw error;
      console.log('☁️ Đã đồng bộ ngầm dữ liệu lên Supabase');
    } catch (e) {
      console.error(e);
      toast('Lỗi đồng bộ Supabase: ' + e.message, 'error');
    }
  },
  async loadFromCloud() {
    ...
    try {
      ...
      if (data && data.settings && Object.keys(data.settings).length > 0) {
        this.books = data.settings.books || [];
        this.chapters = data.settings.chapters || [];
        this.cards = data.settings.cards || [];
        this.dictationPlaylist = data.settings.dictationPlaylist || [];
        this.progress = data.settings.progress || this.progress;
        this.save();
        toast('☁️ Đã tải dữ liệu từ Supabase thành công!', 'success');
